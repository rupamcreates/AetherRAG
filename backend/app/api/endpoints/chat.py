...
class LocalAgentConfig:
    SYSTEM_INSTRUCTIONS = (
        ...
        "7. If and only if a document chunk is explicitly marked with 'Content Type: image_transcription' and contains a visual diagram or layout image, you MUST also output a standard markdown image syntax block: ![Image Description](ID) where ID matches the integer index of that specific image chunk (e.g., ![Transformer Architecture Block Diagram](3) if chunk [^3] is of content type image_transcription). NEVER output an image markdown block for standard text-only chunks.\n"
